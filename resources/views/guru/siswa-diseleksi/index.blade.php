...
@section('title', 'Siswa yang Sudah Diseleksi - PPDB')
                ...
                <div class="card-body p-0">
                    <div class="table-responsive">
                        <table class="table table-hover">
                            ...
                            <tbody>
                                @forelse($siswaDiterima as $index => $siswa)
                                <tr>
                                    <td>{{ ($siswaDiterima->currentPage() - 1) * $siswaDiterima->perPage() + $index + 1 }}</td>
                                    ...
                                    <td>
                                        <span class="status-badge status-{{ $siswa->status_seleksi }}">
                                            {{ ucfirst($siswa->status_seleksi) }}
                                        </span>
                                    </td>
                                    <td>
                                        <form action="{{ route('guru.siswa-diseleksi.update', $siswa->id) }}" method="POST" style="display: inline;">
                                            @csrf
                                            @method('PUT')
                                        ...
                                        <form action="{{ route('guru.siswa-diseleksi.destroy', $siswa->id) }}" method="POST" style="display: inline; margin-left: 5px;" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data siswa ini?')">
                                            @csrf
                                            @method('DELETE')
                    ...
                    <div class="d-flex justify-content-end mt-3">
                        {{ $siswaDiterima->links('pagination::custom') }}
                    </div>
                    @endif
                </div>
            </div>
...
function updateAllToPengumuman() {
    if (confirm('Apakah Anda yakin ingin mengupdate semua data siswa yang diseleksi ke halaman Pengumuman user?')) {
        fetch('{{ route("guru.siswa-diseleksi.update-all") }}', {
            method: 'POST',
            headers: {
